...
RUN mkdir /armory
WORKDIR /armory
ENV ARMORY_VERSION 0.95
RUN curl -SLO "https://github.com/goatpig/BitcoinArmory/releases/download/v${ARMORY_VERSION}.0/armory_${ARMORY_VERSION}_amd64.deb"
RUN curl -SLO "https://github.com/goatpig/BitcoinArmory/releases/download/v${ARMORY_VERSION}.0/sha256sum.txt.asc"
...
RUN gpg --import goatpig-signing-key.asc \
 ...
 && rm "armory_${ARMORY_VERSION}_amd64.deb" sha256sum.txt.asc sha256sum.txt
